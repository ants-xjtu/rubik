...
class GlobalContext:
    ...
    def write_all(self, global_entry: BasicBlock) -> str:
        return (
            ...
            + "\n\n"
            + f"WV_U8 WV_ProcessPacket(WV_ByteSlice packet, WV_Runtime *runtime) "
            + make_block(
    ...
    def write_finalize_block(self):
        return "\n".join(
            [f"if (nf{i}) free(nf{i});" for i in range(self.layer_count)]
            + ["return status;"]
        )
    ...
    def write_inst_types_decl(self) -> str:
        return "\n".join(
            struct.create_aux().declare_inst_type(lid)
    ...
    def write_content_vars(self) -> str:
        return "\n".join(
            f"WV_ByteSlice c{i} = WV_EMPTY;\nWV_Byte *nf{i} = NULL;"
            for i in range(self.layer_count)
        )

    def write_return_vars(self) -> str:
        return "\n".join(
            f"WV_I32 saved_target_{block.block_id};"
            for block in self.required_return_blocks
        )
    ...
    def write_regs() -> str:
        return "\n".join(
            reg_aux.decl(reg_id)
            for reg_id, reg in reg_aux.regs.items()
            if not reg.abstract and not isinstance(reg, StructRegAux)
        )
    ...
    def write_shower(self) -> str:
        targets_text = (
            "\n".join(
                f"case {block.block_id}: goto NI{block.block_id}_Ret;"
                for block in self.required_return_blocks
            )
            + "\ndefault: goto GlobalExit;"
        )
        return "L_Shower: " + make_block(
            f"switch (ret_target) {make_block(targets_text)}"
        )

    def write_extern_calls_decl(self) -> str:
        return "\n".join(
            f'extern WV_U8 {name}({", ".join([reg_aux[reg].type_decl() for reg in regs] + ["WV_Any *"])});'
            for name, regs in self.required_calls.items()
        )

    def write_eq_funcs(self) -> str:
        return "\n".join(
            f"int {GlobalContext.eq_func_name(lid)}(const void *key, const void *object) "
            + make_block(
        ...
        runtime_struct = (
            ...
            + make_block(
                "\n".join(
                    ...
                    + [
                        f"tommy_hashdyn hash_{lid};"
                        for lid in self.required_inst.keys()
                    ]
                )
            )
            + ";"
...
class InstrContext:
    def __init__(
        self, recurse_context: BlockRecurseContext, block: BasicBlock, instr: Instr
    ):
